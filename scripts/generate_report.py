...
def _pdf_text(value) -> str:
    """Return text compatible with Helvetica/core-font PDF output."""
    text = str(value)
    replacements = {
        "\u2013": "-",
        "\u2014": "-",
...
def build_report():
    ...
    _cell(pdf, 0, 15, "prode-ML", ln=True, align="C")
    pdf.set_font("Helvetica", "", 14)
    _cell(pdf, 0, 10, "FIFA World Cup 2026 - Predictions Report", ln=True, align="C")
    pdf.set_font("Helvetica", "I", 10)
    _cell(pdf, 0, 8, f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}", ln=True, align="C")
    pdf.ln(5)

    _load_metadata(pdf)

    # â”€â”€ Group Predictions â”€â”€
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 16)
    _cell(pdf, 0, 12, "1. Group Stage Predictions", ln=True)
    pdf.set_font("Helvetica", "", 9)
    _cell(pdf, 0, 6, "All 72 group matches with predicted probabilities and expected goals.", ln=True)
    pdf.ln(3)
    ...
    for group_name, teams in GROUPS.items():
        ...
        for i, j in fixtures:
            ...
            try:
                ...
                _cell(pdf, 0, 5, f"  {team_a} vs {team_b} -- error: {e}", ln=True)

        pdf.ln(3)

    # â”€â”€ Group Simulation â”€â”€
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 16)
    _cell(pdf, 0, 12, f"2. Group Stage Simulation ({REPORT_SIMULATIONS:,} runs)", ln=True)
    pdf.ln(4)

    for group_name in GROUPS:
        _group_sim_section(pdf, group_name, group_sim)

    # â”€â”€ Tournament Simulation â”€â”€
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 16)
    _cell(pdf, 0, 12, f"3. Tournament Simulation ({REPORT_SIMULATIONS:,} runs)", ln=True)
    pdf.ln(4)
...
def _load_metadata(pdf):
    ...
    if not meta_path.exists():
        pdf.set_font("Helvetica", "", 9)
        _cell(pdf, 0, 6, "No model metadata found. Train models first.", ln=True)
        return
    ...
    _cell(pdf, 0, 6, "Model Metrics:", ln=True)
    pdf.set_font("Helvetica", "", 8)
    ...
    acc = [
        ...
        ("Weighted voting (ensemble)", meta.get("accuracy_voting", 0)),
        ("Confederation", meta.get("accuracy_confederation", 0)),
    ]
    _cell(pdf, 0, 5, "Model Accuracies (test set, calibrated):", ln=True)
    for name, val in acc:
        if val > 0:
            _cell(pdf, 90, 4, f"  {name}", ln=False)
            _cell(pdf, 0, 4, f"{val:.1%}", ln=True)

    ts_acc = meta.get("accuracy_two_stage", 0)
    baseline = meta.get("baseline_random", 0.333)
    if ts_acc > 0:
        note = " (below random - diagnostic only)" if ts_acc < baseline else " (diagnostic only)"
        _cell(pdf, 90, 4, f"  Two-Stage{note}", ln=False)
        _cell(pdf, 0, 4, f"{ts_acc:.1%}", ln=True)

    acc_elo = meta.get("accuracy_high_elo_diff_200", 0)
    n_elo = meta.get("n_high_elo_matches", 0)
    _cell(pdf, 0, 4, f"  High Elo diff (>=200): {acc_elo:.1%} (n={n_elo})", ln=True)
    _cell(pdf, 0, 4, f"  Baseline random: {meta.get('baseline_random', 0.333):.1%}", ln=True)
...
def _group_header(pdf, group_name, teams):
    ...
    _cell(pdf, 0, 7, f" GROUP {group_name}: {', '.join(teams)} ", ln=True, fill=True)
    pdf.set_text_color(0, 0, 0)
    pdf.ln(1)

    pdf.set_font("Helvetica", "B", 7)
    _cell(pdf, 48, 5, "Match", ln=False, border=1)
    _cell(pdf, 18, 5, "Win A", ln=False, align="C", border=1)
    _cell(pdf, 18, 5, "Draw", ln=False, align="C", border=1)
    _cell(pdf, 18, 5, "Win B", ln=False, align="C", border=1)
    _cell(pdf, 24, 5, "xG", ln=False, align="C", border=1)
    _cell(pdf, 24, 5, "Most Likely", ln=False, align="C", border=1)
    _cell(pdf, 20, 5, "Conf", ln=False, align="C", border=1)
    _cell(pdf, 20, 5, "Upset", ln=True, align="C", border=1)
...
def _match_row(pdf, team_a, team_b, result):
    ...
    _cell(pdf, 48, 5, match_text, ln=False, border=1)

    _cell(pdf, 18, 5, f"{result['p_win_a']:.1%}", ln=False, align="C", border=1)
    _cell(pdf, 18, 5, f"{result['p_draw']:.1%}", ln=False, align="C", border=1)
    _cell(pdf, 18, 5, f"{result['p_win_b']:.1%}", ln=False, align="C", border=1)

    xg_text = f"{result['xg_a']:.1f}-{result['xg_b']:.1f}"
    _cell(pdf, 24, 5, xg_text, ln=False, align="C", border=1)

    sl = result.get("top_scorelines", [])
    if sl:
        s = sl[0]
        if isinstance(s, dict):
            goals_a = int(s.get("goals_a", 0))
            goals_b = int(s.get("goals_b", 0))
            probability = float(s.get("probability", 0))
        else:
            goals_a = int(s[0])
            goals_b = int(s[1])
            probability = float(s[2])
        score_text = f"{goals_a}-{goals_b} ({probability:.1%})"
    else:
        score_text = "-"
    _cell(pdf, 24, 5, score_text, ln=False, align="C", border=1)

    conf = result.get("confidence", "?")
    conf_icon = ALTO if conf == "ALTO" else (MEDIO if conf == "MEDIO" else BAJO)
    _cell(pdf, 20, 5, f"{conf_icon} {conf}", ln=False, align="C", border=1)

    upset = result.get("upset_risk", 0)
    _cell(pdf, 20, 5, f"{upset:.1%}", ln=True, align="C", border=1)
...
def _group_sim_section(pdf, group_name, group_sim):
    ...
    pdf.set_fill_color(30, 60, 120)
    pdf.set_text_color(255, 255, 255)
    _cell(pdf, 0, 7, f" GROUP {group_name} ", ln=True, fill=True)
    pdf.set_text_color(0, 0, 0)
    pdf.ln(1)
    ...
    for h, w in zip(headers, widths):
        _cell(pdf, w, 5, h, ln=False, align="C", border=1)
    pdf.ln()
    ...
    for _, row in group_df.iterrows():
        ...
        for v, w in zip(vals, widths):
            _cell(pdf, w, 5, v, ln=False, align="C", border=1)
        pdf.ln()

    try:
        fixtures = group_sim.simulate_group_fixtures(group_name, n_sims=REPORT_SIMULATIONS)
        if fixtures:
            pdf.ln(2)
            pdf.set_font("Helvetica", "B", 7)
            _cell(pdf, 0, 5, "  Most Likely Results:", ln=True)
            pdf.set_font("Helvetica", "", 7)
            for fix in fixtures:
...
def _tournament_section(pdf, tourn_df):
    tourn_df = tourn_df.sort_values("p_champion", ascending=False).head(20)
    ...
    _cell(pdf, 0, 5, "Note: Knockout rounds use Poisson-sampled results. Draws resolved by Elo-weighted penalty shootout.", ln=True)
