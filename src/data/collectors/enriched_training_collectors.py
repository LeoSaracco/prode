...
class KaggleMatchFeaturesCollector:
    ...
    def collect(self) -> pd.DataFrame:
        ...
        home_col = _first_present(raw.columns, ["home_team", "home", "team_a", "team1"])
        away_col = _first_present(raw.columns, ["away_team", "away", "team_b", "team2"])
        home_goals = _first_present(raw.columns, ["home_score", "home_goals", "goals_home", "home_team_goal", "score_home"])
        away_goals = _first_present(raw.columns, ["away_score", "away_goals", "goals_away", "away_team_goal", "score_away"])
        date_col = _first_present(raw.columns, ["date", "match_date"])
        tournament_col = _first_present(raw.columns, ["tournament", "competition", "competition_name"])
        neutral_col = _first_present(raw.columns, ["neutral", "is_neutral"])
